/**
 * 前端 MD5 工具（RFC 1321 实现，纯 TS 无依赖）
 *
 * 需求文档安全要求"前端加盐 MD5 + 后端 MD5 双重加密"：
 * 前端对明文做一次 MD5 摘要后再提交，后端再对收到的摘要做一次 MD5 后存储/校验，
...
function wordToHex(v: number): string {
  let out = ''
  for (let i = 0; i < 4; i++) {
    out += ((v >>> (i * 8)) & 0xff).toString(16).padStart(2, '0')
  }
  return out
}

/** 计算字符串的 MD5（按 UTF-8 编码） */
export function md5(input: string): string {
  // 将 Unicode 字符串转为 UTF-8 字节串（Latin-1 视作字节）
